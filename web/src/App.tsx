...
import type { AuditView, DataHealthView, GateView, OverviewView, PortfolioView, RiskView, StrategyValidationView, TradeLeadView, WatchlistView, ExperimentReportingView, IncidentReportingView, RecoveryReportingView } from './lib/models';
...
type Page = 'overview' | 'leads' | 'watchlist' | 'portfolio' | 'risk' | 'validation' | 'experiments' | 'incidents' | 'recovery' | 'gates' | 'health' | 'audit';

const NAV: Array<[Page, string]> = [
  ['overview','Overview'], ['leads','Trade Leads'], ['watchlist','Watchlist'], ['portfolio','Portfolio'],
  ['risk','Risk'], ['validation','Validation'], ['experiments','Experiments'], ['incidents','Incidents'], ['recovery','Recovery'], ['gates','Phase Gates'], ['health','Data Health'], ['audit','Audit Trail']
];
...
export default function App() {
  ...
  const [incidents, setIncidents] = useState<IncidentReportingView | null>(null);
  const [recovery, setRecovery] = useState<RecoveryReportingView | null>(null);

  useEffect(() => {
    Promise.all([
      researchApi.overview(), researchApi.leads(), researchApi.watchlist(), researchApi.gates(),
      researchApi.dataHealth(), researchApi.portfolio(), researchApi.risk(), researchApi.audit(), researchApi.strategyValidation(), researchApi.experiments(), researchApi.incidents(), researchApi.recovery()
    ]).then(([o,l,w,g,h,p,r,a,v,e,i,rc]) => { setOverview(o); setLeads(l); setWatchlist(w); setGates(g); setHealth(h); setPortfolio(p); setRisk(r); setAudit(a); setValidation(v); setExperiments(e); setIncidents(i); setRecovery(rc); })
      .catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Unable to read research API'));
  }, []);

  return <div className="app-shell"><aside><div className="brand"><div className="brand-mark">QT</div><div><strong>Quant Platform</strong><span>Research Console</span></div></div><nav>{NAV.map(([key,label]) => <button key={key} className={page === key ? 'active' : ''} onClick={() => setPage(key)}>{label}</button>)}</nav><div className="side-footer"><span>PHASE 02</span><strong>READ ONLY</strong><small>No broker connectivity</small></div></aside><main><header><div><span className="system-dot" /> Local research environment</div><div className="header-meta"><span>CSMOM-LS v0.2</span><strong>Phase 03 locked</strong></div></header><div className="content">{error ? <div className="error-panel">{error}</div> : !overview ? <div className="loading">Loading deterministic research state…</div> : page === 'overview' ? <OverviewPage overview={overview} gates={gates} health={health} /> : page === 'leads' ? <LeadsPage leads={leads} /> : page === 'watchlist' ? <WatchlistPage entries={watchlist} /> : page === 'portfolio' && portfolio ? <PortfolioPage data={portfolio} /> : page === 'risk' && risk ? <RiskPage data={risk} /> : page === 'validation' && validation ? <ValidationPage data={validation} /> : page === 'experiments' && experiments ? <ExperimentsPage data={experiments} /> : page === 'incidents' && incidents ? <IncidentsPage data={incidents} /> : page === 'recovery' && recovery ? <RecoveryPage data={recovery} /> : page === 'gates' ? <GatesPage gates={gates} /> : page === 'health' ? <DataHealthPage health={health} /> : <AuditPage records={audit} />}</div></main></div>;
}
